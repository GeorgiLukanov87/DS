"""
Build an ETL pipeline with Python | Data pipeline | Export from SQL Server to PostgreSQL
"""
import os
...
def extract():
    try:
        ...
        from sys.tables t where t.name in
         ('UserGameItems','Characters','Games','Items','UserGameItems','Users','UsersGames') """
...
def load(df, tbl):
    try:
        rows_imported = 0
        engine = create_engine(f'postgresql://{uid}:{pwd}@{server}:5432/test_db')
